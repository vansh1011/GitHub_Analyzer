...
export function Home() {
  ...
  if (total_stars.current > 100) rank = "Developer ⚡";
  if (total_stars.current > 500) rank = "Expert 🔥";
  if (total_stars.current > 1000) rank = "Legend 👑";
  if (total_stars.current > 1000) rank = "Open Source King 👑";
    ...
    <div className="min-h-screen bg-gray-900 text-white flex flex-col md:flex-row gap-6 md:gap-8 p-4 md:p-8">
      <div className="flex-1">
        ...
        <div className="bg-gray-800 rounded-2xl p-4 md:p-5 shadow-lg space-y-2 mb-6">
          ...
          <div>
            Login : <span className="font-bold">{data.login}</span>
          </div>
          <div>
            Public Repo : <span className="font-bold">{data.public_repos}</span>
          </div>
        </div>

        <div className="bg-linear-to-r from-purple-600 to-blue-600 rounded-2xl p-4 md:p-5 shadow-lg mb-6">
          <h2 className="text-xl font-bold">{rank}</h2>

          <div className="mt-2">Total Stars : {total_stars.current}</div>

          <div>Total Forks : {total_forks.current}</div>

          <div>Top Language : {top_language.current || "N/A"}</div>
        </div>
            ...
            <div className="space-y-4 md:space-y-5">
              {repos.map((repo) => (
                ...
                >
                  ...
                  <div className="mb-1">Language : {repo.language}</div>

                  <div className="mb-1">Fork count : {repo.forks_count}</div>

                  <div className="mb-3">Stars : {repo.stargazers_count}</div>
